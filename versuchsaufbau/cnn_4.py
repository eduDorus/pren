...
network = conv_2d(network, 40, 7, strides=3, activation='relu')
network = max_pool_2d(network, 2)
network = conv_2d(network, 80, 5, strides=2, activation='relu')
network = conv_2d(network, 160, 3, strides=1, activation='relu')
...
model.fit(X, Y, n_epoch=10, shuffle=True, validation_set=(X_test, Y_test),
          show_metric=True, batch_size=200, run_id='model_4')
